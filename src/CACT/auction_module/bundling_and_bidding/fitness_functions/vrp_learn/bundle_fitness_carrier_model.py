...
from auction_module.bundling_and_bidding.fitness_functions.bundle_fitness import (
    BundleFitnessFunction,
)
from auction_module.bundling_and_bidding.fitness_functions.vrp_learn.optimization_policy import (
    OptimizationPolicy,
)
from auction_module.bundling_and_bidding.fitness_functions.vrp_learn.target_function import (
    TargetFunction,
)
...
class _CarrierModel:
    def __init__(
        ...
        self.params_names: tuple[str] = tuple(
            [f"x{i}" for i in range(self._num_unknown_orders)]
            + [f"y{i}" for i in range(self._num_unknown_orders)]
        )
        self._current_params: dict[str, Optional[float]] = {
            k: None for k in self.params_names
        }
    ...
    def get_coords(self) -> list[tuple[float, float]]:
        x_coords = [
            self.current_params[f"x{i}"] for i in range(self._num_unknown_orders)
        ]
        y_coords = [
            self.current_params[f"y{i}"] for i in range(self._num_unknown_orders)
        ]
        ...
        if self._current_params_have_changed or not self._requests:
            ...
            for i, (x, y) in enumerate(self.get_coords()):
                request = Request(
                    ...
                    disclosure_time=datetime.datetime.min,
                    revenue=1,
                    load=1,
                    # TODO these values are rather arbitrary
                    service_duration=timedelta(minutes=4),
                    tw_open=self._depot.tw_open,  # same time windows as depot, i.e. no time window
                    tw_close=self._depot.tw_close,
                )
    ...
    def _current_params_instance(self, original_instance: CAHDInstance) -> CAHDInstance:
        ...
        :return:
        """
        own_requests = self.requests(
            original_instance.num_vertices, original_instance.num_requests
        )
        extended_requests = original_instance.requests + own_requests
        extended_xy = np.array(
            [(v.x, v.y) for v in original_instance.vertices + own_requests]
        )
        if original_instance.meta["type"] == "euclidean":
            extended_distance_matrix = squareform(
                pdist(extended_xy, metric="euclidean")
            )
            ...
            raise NotImplementedError(
                "Not yet correctly implemented for vienna type instances. OSRM distances are not"
            ...
            inst_travel_duration = total_seconds_vectorized(
                original_instance._travel_duration_matrix
            )
            dist_scaling_matrix = np.divide(
                inst_travel_duration,
                inst_eucl_dist,
                out=np.zeros_like(inst_travel_duration),
            ...
            duration_matrix = np.vectorize(lambda x: timedelta(seconds=x))(
                duration_matrix
            )
    ...
    def _carrier_without_bundle(self, original_instance: CAHDInstance):
        ...
        :return:
        """
        carrier_without_bundle = Carrier(
            self._carrier_idx,
            f"model of carrier {self._carrier_idx}",
            self._depot,
            self._objective,
            self._num_vehicles,
            self._routing_solver,
        )
    ...
    def compute_bid_on_bundles(
        self, original_instance: CAHDInstance, bundles: Sequence[Bundle]
    ):
        """
        ...
        :return:
        """
        current_params_instance = self._current_params_instance(
            original_instance
        ...
        for query in bundles:
            bid = carrier_without_bundle.compute_bid_on_bundle(
                current_params_instance, query
            )
...
class BundleFitnessCarrierModel(BundleFitnessFunction):
    def __init__(
        ...
        self._prediction_metrics = (
            [] if prediction_metrics is None else prediction_metrics
        )
        self._params = {
