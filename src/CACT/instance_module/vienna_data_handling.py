...
def read_vienna_addresses(
        path=io.cr_ahd_instances_dir.joinpath('viena_instances/instance_creation/vienna_addresses.csv'),
        EPSG=4326,
        **kwargs
):
    """
    reads the address data from a csv file and parses datetime data, transforms geometries to proper shapely object,
    puts all data into a GeoPandas GeoDataFrame and filters out locations such as train stations, museums, ...
    ...
    gdf = gdf[gdf['GEB_BEZEICHNUNG'].isna()]
    gdf = gdf[~gdf['NAME'].str.contains('Parz')]
    gdf = gdf[~gdf['NAME'].str.contains('KLG')]
    return gdf


def _query_vienna_addresses_online(
        write_path: Path = io.cr_ahd_instances_dir.joinpath('viena_instances/instance_creation/vienna_addresses.csv'),
    ...
    for district in districts:
        ...
        # some cleaning since there was at least one record with a transposition of digits in the ZIP code
...
def query_osrm(gs: gp.GeoSeries, mode: str, api_limit=100):
    ...
    :param mode: either of 'duration' or 'distance
    :param gs:
...
if __name__ == '__main__':
    # re-query the data from https://www.data.gv.at/katalog/dataset/stadt-wien_adressdatenderstadtwien
    ...
    gdf_full = read_vienna_addresses(
        io.cr_ahd_instances_dir.joinpath('viena_instances/instance_creation/vienna_addresses.csv'),
    ...
    for i in range(3):
        ...
        write_path = io.unique_path(io.cr_ahd_instances_dir.joinpath('viena_instances/instance_creation'),
                                    f'vienna_{n}_addresses' + '_#{:03d}' + '.csv')
        ...
        gdf.to_csv(io.cr_ahd_instances_dir.joinpath('viena_instances/instance_creation').joinpath(write_path),
                   encoding='utf-8-sig', index=True, header=True)
