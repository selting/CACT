...
def read_vienna_districts_shapefile():
    districts = gp.read_file(io.vienna_instance_creation_dir.joinpath('BEZIRKSGRENZEOGD/BEZIRKSGRENZEOGDPolygon.shp'))
...
def routed_request_marker(instance, carrier, color, index, tour, vertex):
    ...
    return folium.CircleMarker(
        ...
        popup=f'Request {request}<br>'
              ...
              f'service time={tour.service_time_sequence[index]}<br>'
              f'service_duration={tour.service_duration_sequence[index]}<br>'
...
def depot_marker(instance, carrier, color):
    return folium.features.RegularPolygonMarker(location=instance.coords(carrier.depot_vertex),
                                                number_of_sides=4,
                                                popup=f'Depot of carrier {carrier.id_}<br>'
                                                      f'dur={carrier.sum_travel_duration}<br>'
                                                      f'dist={round(carrier.sum_travel_distance)}',
                                                tooltip=f'Depot {carrier.depot_vertex}',
                                                rotation=45,
                                                radius=10,
                                                color='black',
                                                fill_color=color,
...
def plot_carrier_routes(instance, solution, carrier_ids, title=None):
    for carrier_id in carrier_ids:
        carrier = solution.carriers[carrier_id]
        for tour in carrier.tours:
            coordinates = []
            labels = []
            for vertex in tour.routing_sequence:
                coords = instance.coords(vertex)
                coordinates.append(coords)
                labels.append(vertex)
            x_coords, y_coords = zip(*coordinates)
            # tour
            plt.plot(x_coords, y_coords, marker='o', markersize=4, linewidth=1)
            # labels
            for i, txt in enumerate(labels):
                plt.annotate(txt, (x_coords[i], y_coords[i]))
            # depot
            plt.plot(x_coords[0], y_coords[0], marker='s', markersize=6, color='black')

    # text box in upper right corner with every carrier's total travel duration and distance
    textstr = ''
    for carrier_id in carrier_ids:
        carrier = solution.carriers[carrier_id]
        textstr += f'Carrier {carrier_id}:\n'
        textstr += f'  duration: {carrier.sum_travel_duration.total_seconds()}\n'
        # textstr += f'  distance: {round(carrier.sum_travel_distance)}\n'
    props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
    plt.text(0.05, 0.95, textstr, transform=plt.gca().transAxes, fontsize=8,
    ...
    if title:
        ...
        plt.title(f'Route plans of carriers {carrier_ids}')

    # limits
    plt.xlim([48.11832, 48.32248])
    plt.ylim([16.18212, 16.57771])
