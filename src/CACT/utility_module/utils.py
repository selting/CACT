...
univie_colors_100_60 = univie_colors_100 + univie_colors_60

# paired
univie_colors_paired = list(itertools.chain(*zip(univie_colors_100, univie_colors_60)))

univie_cmap = LinearSegmentedColormap.from_list(
    "univie", univie_colors_100, N=len(univie_colors_100)
)
univie_cmap_paired = LinearSegmentedColormap.from_list(
    "univie_paired",
    univie_colors_paired,
...
def map_to_univie_colors(categories: Sequence):
    ...
    for cat, color in zip(
        categories, itertools.cycle(univie_colors_100 + univie_colors_60)
    ):
        colormap[cat] = color
    return colormap
...
def split_iterable(iterable, num_chunks):
    ...
    return (
        iterable[i * k + min(i, m) : (i + 1) * k + min(i + 1, m)]
...
def argsmin(a, k: int):
    ...
    assert k > 0
    a_sorted = sorted(
        range(len(a)), key=lambda x: a[x]
    )  # indices in increasing order of values
    return a_sorted[:k]
...
def argsmax(a, k: int):
    ...
    assert k > 0
    a_sorted = sorted(
        range(len(a)), key=lambda x: a[x], reverse=True
...
def midpoint(instance, pickup_vertex, delivery_vertex):
    pickup_x, pickup_y = (
        instance.vertex_x_coords[pickup_vertex],
        instance.vertex_y_coords[delivery_vertex],
    )
...
def linear_interpolation(
    iterable: Sequence, new_min: float, new_max: float, old_min=None, old_max=None
):
    """
...
def indices_to_nested_lists(indices: Sequence[int], elements: Sequence):
    """
    sorts elements in lists based on the index provided.

    Example:
        >>> indices = [1, 0, 0, 2, 1, 2]; elements = [0, 1, 2, 3, 4, 5]
        >>> indices_to_nested_lists(indices, elements)
        [[1, 2], [0, 4], [3, 5]]
...
def validate_tour(instance, tour):
    assert tour.sum_load == sum(
        [v.load for v in tour.routing_sequence if isinstance(v, Request)]
    ), instance.id_
    ...
    for i in trange(
        1, len(tour.routing_sequence), desc=f"Tour {tour.id_}", disable=True
    ):
        ...
        assert tour.arrival_time_sequence[i] == tour.service_time_sequence[
            i - 1
        ] + predecessor.service_duration + instance.travel_duration(
            [predecessor], [vertex]
        ), msg

        # waiting times
        assert (
            tour.service_time_sequence[i]
            == tour.arrival_time_sequence[i] + tour.wait_duration_sequence[i]
        ), msg
        assert tour.wait_duration_sequence[i] == max(
            dt.timedelta(0), vertex.tw_open - tour.arrival_time_sequence[i]
        ), msg

        # max_shift times
        if not isinstance(vertex, Depot):
            assert tour.max_shift_sequence[i] == min(
                vertex.tw_close - tour.service_time_sequence[i],
                tour.wait_duration_sequence[i + 1] + tour.max_shift_sequence[i + 1],
            ), msg
        else:
            assert (
                tour.max_shift_sequence[i]
                == vertex.tw_close - tour.service_time_sequence[i]
            )

        # tw constraint
        assert vertex.tw_open <= tour.service_time_sequence[i] <= vertex.tw_close, msg
...
#
# def my_logify_dict(obj: dict[str, Any], logify_datetime_obj=False, sep='__'):
#     logified = {}
#     for k, v in obj.items():
#         logified[k] = my_logify(v, logify_datetime_obj)
#         if hasattr(v, 'params') and isinstance(v.params, dict) and bool(v.params):
#             logified[k] = v.__class__.__name__
#             logified_params = my_logify_dict(v.params)
#             logified.update({f'{k}{sep}{param}': param_value for param, param_value in logified_params.items()})
