...
function combine_prefixed_columns!(df::DataFrame, prefix::String; new_col::Symbol=Symbol(prefix * "_combined"))
    ...
    df[!, new_col] = map(eachrow(df)) do row
        parts = ["$(sn)=$(row[c])" for (sn, c) in zip(short_names, matching_cols)]
...
function load_results(paths, key="optimize_result")
    ...
    single_step_fields = [:x_opt, :opt_val, :num_evals]
...
params_df = collect_results(datadir("exp_raw"), black_list=["input_data", "optimize_result"])
...
@df ssdf StatsPlots.boxplot(string.(:optimizer), :opt_val,)
@df ssdf StatsPlots.dotplot!(string.(:optimizer), :opt_val)


# group by some grouping vars
group_cols = [
...
function plot_bands(df; x=:step, y=:proxy_objective, color=nothing, col=nothing, row=nothing)
    y_mean = Symbol(string(y)*"_mean")
    y_lower=Symbol(string(y)*"_lower")
    y_upper=Symbol(string(y)*"_upper")

    # build mapping kwargs dynamically, skipping any that are `nothing`
    band_kwargs = Dict{Symbol,Any}()
    line_kwargs = Dict{Symbol,Any}()
    color !== nothing && (band_kwargs[:color]=color; line_kwargs[:color]=color)
    col !== nothing && (band_kwargs[:col]=col; line_kwargs[:col]=col)
    row !== nothing && (band_kwargs[:row]=row; line_kwargs[:row]=row)

    band_layer = data(df) *
                 mapping(x, y_lower, y_upper; band_kwargs...) *
                 visual(Band; alpha=0.3)
    line_layer = data(df) *
                 mapping(x, y_mean; line_kwargs...) *
                 visual(Lines)

    plt = band_layer + line_layer
    draw(plt; legend=(position=:bottom, orientation=:vertical, titleposition=:left))
end

plot_bands(agg_msdf; y=:proxy_objective, color=:optimizer_combined)
plot_bands(agg_msdf, y=:incumbent_proxy_objective, color=:optimizer_combined)
plot_bands(agg_msdf; y=Symbol("true_objectives__HausdorffDistance"), color=:optimizer_combined)


plt = data(msdf) *
      mapping(:step, :proxy_objective, color=:x0_seeder, col=:optimizer, row=:optimizer__max_eval) *
      visual(Lines)
draw(plt)
...
# long_df = combine(gdf) do sdf
#     (optimize_result_df = optimize_result_2_df.(sdf.optimize_result))
# end